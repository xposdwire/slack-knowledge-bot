...
def fetch_recent_messages(channel_id, count=200, days_back=None):
    try:
        latest = datetime.now()
        ...
        messages = [m for m in all_messages if m.get("subtype") != "bot_message"]
...
def fetch_messages_around_time(channel_id, target_time, minutes=10):
    ...
    start = (target_time - delta).timestamp()
    end = (target_time + delta).timestamp()
    messages = message_cache.get(channel_id, [])
    return [m for m in messages if start <= float(m['ts']) <= end and 'user' in m]
...
def resolve_user_name(name):
    try:
        ...
        for user in users:
            ...
            if name in {uid.lower(), uname, display, real}:
                return uid
    except SlackApiError as e:
...
def handle_app_mention(body, say):
    ...
    if match:
        ...
        if hits:
            ...
            timestamp = datetime.fromtimestamp(float(closest['ts'])).strftime('%I:%M %p on %B %d')
